...
"""
================================================================================
LLM OUTPUT TEXT PARAMETERS ANALYSIS
...
def create_custom_model(base_model: str, cfg: dict) -> Optional[str]:
    """Create a custom model via Ollama API Modelfile."""
    safe  = base_model.replace(":", "-").replace(".", "-")
    name  = f"{safe}-{cfg['suffix']}"

    modelfile = (
        f"FROM {base_model}\n\n"
        f"PARAMETER num_predict {cfg['num_predict']}\n"
        f"PARAMETER temperature {cfg['temperature']}\n"
        f"PARAMETER top_p {cfg['top_p']}\n"
        f"PARAMETER repeat_penalty {cfg['repeat_penalty']}\n"
        f"PARAMETER num_ctx 131072\n\n"
        f"SYSTEM \"\"\"{cfg['system']}\"\"\"\n"
    )
...
def delete_custom_model(name: str) -> None:
    """Delete a custom model after use to free disk space."""
    try:
        requests.delete(
...
def generate(model: str, prompt: str, cfg: dict) -> dict:
    """Stream generation from Ollama, return text + stats."""
    full_prompt = (
        "# Large Language Models: Architecture, Training, and Applications\n\n"
...
def print_summary(results: List[dict]) -> None:
    ...
    print(f"Saved to: {JSON_FILE}  |  {CSV_FILE}")
...
def main():
    ...
    print("=" * 65)
    print("  LLM TEXT PARAMETERS ANALYSIS")
    ...
    print(f"[+] Total records to generate: {len(active_models)} × {len(CONFIGS)} × {len(PROMPTS)} = "
          f"{len(active_models) * len(CONFIGS) * len(PROMPTS)}")
    ...
    for base_model in active_models:
        ...
        for cfg in CONFIGS:
            # Create custom model for this config
            print(f"\n  Config: {cfg['label']} | T={cfg['temperature']} | "
                  f"top_p={cfg['top_p']} | max_tokens={cfg['num_predict']}")
            custom_name = create_custom_model(base_model, cfg)
            if not custom_name:
                print(f"  [!] Skipping config {cfg['label']} — model creation failed")
                continue
            ...
            for prompt_key, prompt_text in PROMPTS.items():
                ...
                gen    = generate(custom_name, prompt_text, cfg)
                ...
                print(f"    AvgSent:  {params['avg_sentence_len']:.1f}")
                print(f"    Disc:     {params['discourse_markers']}")
                print(f"    TPS:      {gen['tokens_per_sec']:.1f}")

                # Save after every record
                save_results(results)

            # Delete custom model to free disk space
            delete_custom_model(custom_name)
